...
define('WP_SAVORY_OPTS', 'wp_savory');

if (!defined('PLUGIN_URL_savory'))
	define('PLUGIN_URL_savory', WP_PLUGIN_URL . '/savory');
define('PLUGIN_PATH_savory', WP_PLUGIN_DIR . '/savory');

// create an instance of the plugin
global $savory_plugin;
if (!$savory_plugin)
	$savory_plugin = new WpSavoryPlugin();

/*
* WpSavoryPlugin Class
*/
class WpSavoryPlugin {

	public $plugin_version = '0.1.883';
	/*
	* constructor
	*/
	function WpSavoryPlugin() {
	...
	*/
	function get_options() {
		...
		$options = get_option(WP_SAVORY_OPTS);	
		if (!empty($options)) {
			foreach ($options as $key => $option)
				$set_options[$key] = $option;
		}else{
			update_option(WP_SAVORY_OPTS, $set_options);
		}
		return $set_options;
	}		
	...
	*/
	function add_pages() {
		$page = add_options_page('Savory', 'Savory', 'install_plugins', basename(__FILE__), array(&$this, 'print_admin'));
	}	
	...
	*/
    ...
    if (method_exists($this, 'before_print_admin'))
      ...
      update_option(WP_SAVORY_OPTS, $options);
?>
<div class="updated"><p><strong><?php _e('Settings Updated.', 'savory');?></strong></p></div>
<?php
		} 
?>
<div class="wrap">
<h2>Savory Plugin v.<?php echo $this->plugin_version; ?></h2>
<?php do_action( 'savory_settings_messages' ); ?>
<form enctype="multipart/form-data" method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
<?php wp_nonce_field('update-options'); ?>
<h3><?php _e('Basic Options', 'savory');?></h3>
<p><?php _e('Select the options which suit your site.', 'savory');?></p>
...
// END auto-generated code
  ...
  function custom_admin_init() {    
    add_action('savory_settings_messages', array(&$this, 'save_settings'));
   ...
   */
	function register_custom_post_type() {		    
		register_post_type( SAVORY_POST_TYPE,
			array(
				'labels' => array(
					'name' => __( 'Savory Items' ),
					'singular_name' => __( 'Savory Item' ),
					'add_new' => __( 'Add New' ),
					'add_new_item' => __( 'Add New Savory Item' ),
					'edit' => __( 'Edit' ),
					'edit_item' => __( 'Edit Savory Item' ),
					'new_item' => __( 'New Savory Item' ),
					'view' => __( 'View Savory Item' ),
					'view_item' => __( 'View Savory Item' ),
					'search_items' => __( 'Search Savory Items' ),
					'not_found' => __( 'No Savory Items found' ),
					'not_found_in_trash' => __( 'No Savory Items found in Trash' )
				),
